...
namespace advent_of_code_2021
{
    internal class Day8
    {
        internal void Part1()
        {
            var validSegmentCounts = new int[] { 2, 3, 4, 7 };
            var specialDigitsCount = _outputs.Select(o => o.Where(x => validSegmentCounts.Contains(x.Length)).Count()).Sum();
            System.Console.WriteLine(specialDigitsCount);
        }
        ...
        internal void Part2()
        {
            ...
            {
                var digits = AnalyzeSignals(line.Split(" | ")[0]);
                var outputNumber = "";
                foreach (var output in line.Split(" | ")[1].Split(" "))
                {
                    var sortedSegments = string.Concat(output.OrderBy(s => s));
                    outputNumber += digits[sortedSegments].ToString();
                }
                sum += int.Parse(outputNumber);
            }
            System.Console.WriteLine(sum);
        }

        private Dictionary<string, int> AnalyzeSignals(string signalString)
        {
            var remainingSignals = new List<string>(signalString.Split(" ").Select(s => string.Concat(s.OrderBy(l => l))));
            var digitToSignal = new Dictionary<int, string>();
            var signalToDigit = new Dictionary<string, int>();
            ...
            // '6' is the 6-segment digit with 1 overlapping segments with '1'.
            foreach (var x in remainingSignals.Where(y => y.Length == 6))
            {
                var overlap = x.Where(l => digitToSignal[1].Contains(l)).Count();
                if (overlap == 1)
                {
                    digitToSignal[6] = x;
                    signalToDigit[x] = 6;
                    remainingSignals.Remove(x);
                    break;
                }
            }

            // '5' is the 5-segment digit with 5 overlapping segments with '6'.
            foreach (var x in remainingSignals.Where(y => y.Length == 5))
            {
                var overlap = x.Where(l => digitToSignal[6].Contains(l)).Count();
                if (overlap == 5)
                {
                    digitToSignal[5] = x;
                    signalToDigit[x] = 5;
            ...
            // '2' is the 5-segment digit with 3 overlapping segments with '5'.
            foreach (var x in remainingSignals.Where(y => y.Length == 5))
            {
                var overlap = x.Where(l => digitToSignal[5].Contains(l)).Count();
                if (overlap == 3)
                {
                    digitToSignal[2] = x;
                    signalToDigit[x] = 2;
            ...
            // '3' is the remaining 5 segment digit.
            s = remainingSignals.Single(y => y.Length == 5);
            digitToSignal[3] = s;
            signalToDigit[s] = 3;
            remainingSignals.Remove(s);

            // '9' is the 6-segment digit with 5 overlapping segments with '5'.
            foreach (var x in remainingSignals.Where(y => y.Length == 6))
            {
                var overlap = x.Where(l => digitToSignal[5].Contains(l)).Count();
                if (overlap == 5)
            ...
            // '0' is the final signal
            s = remainingSignals.Single();
            digitToSignal[0] = s;
            signalToDigit[s] = 0;
        ...
        private static IEnumerable<IEnumerable<string>> _signals => _lines.Select(l => l.Split(" | ")[0].Split(" "));
        private static IEnumerable<IEnumerable<string>> _outputs => _lines.Select(l => l.Split(" | ")[1].Split(" "));
        private static IEnumerable<string> _lines => _data.Split("\r\n");
        private static readonly string _data = @"dac abcf ac fdbcga dgcbae gcbfde fgcbd agfed adcgf cdbgfea | cbfa bcafdg cbfa bafcgd
